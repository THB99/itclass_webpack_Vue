<template>
  <div id="app">
    <el-container>
      ...
      <el-main>
        <h1 style="font-size: 30px;height: 50px">购物车</h1>
          <el-table
            ref="multipleTable"
            v-loading="loading"
            :data="tableData"
...
<!--            <el-button @click="toggleSelection()">取消选择</el-button>-->
            <el-button @click="save()">结算</el-button>
          </div>
...
export default {
  ...
  methods: {
    save:function (){
      ...
      if(rowData.length==0){
        this.$message.error('请选择购物车里的课程');
      }else {
        this.$axios({
          method:'post',
          url:'http://localhost:8099/buyCart',
          headers : {
    ...
    fetchData:function (){
      ...
      if (sessionStorage.getItem("islogin")==null||sessionStorage.getItem("islogin")==0){
        this.$confirm('您还未登录无法查看购物车, 是否登录?', '提示', {
        ...
        this.$axios({
          method:'get',
          url:'http://localhost:8099/getCart?id='+sessionStorage.getItem("islogin")
        }).then(function (resp){
